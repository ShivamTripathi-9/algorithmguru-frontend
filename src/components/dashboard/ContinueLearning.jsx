...
export default function ContinueLearning() {
  ...
  const fetchActiveProjects = async () => {
    try {
      ...
      // Assumption: backend response kuch aisa dega
      // { activeProjects: [...] } ya seedha { data: { activeProjects: [...] } }
      // Agar tumhara backend "in_progress_projects" ya kuch aur naam use karta hai
      // to bas yahan field name change kar dena.
      const list =
        res.data?.activeProjects ||
        res.data?.data?.activeProjects ||
        res.data?.in_progress_projects ||
        [];
    ...
    <div className="rounded-2xl border border-[#16223A]/8 bg-white p-6">
      ...
      {activeProjects.length === 0 ? (
        <p className="mt-8 text-sm text-[#5B6E8C]">
          Abhi tak koi project start nahi kiya hai.
        </p>
      ) : (
